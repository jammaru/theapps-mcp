...
2. ID の種類を確認する
   ...
   - 1回払い決済 → `payment_id` → `apps_get_charge`
   - 定期払い決済 → `payment_id` → `apps_get_paid_payment`
   - 分割払い決済 → `payment_id` → `apps_get_installments_payment`
3. 結果を要約（個人情報の生ログは残さない）

## payment_id が管理画面の数字だけのとき

「Webhook 決済成功の payment_id が必要。管理画面表示 ID では取れない場合がある」と伝え、正しい ID を求める。推測で別 API を連続試行しない。
